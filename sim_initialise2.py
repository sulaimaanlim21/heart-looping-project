...
from force_functions2 import total_force_on_vertex,pressure_force,bending_force2,grow_angle,bending_force3,grow_factor_z
...
def sim_initialise(shape,tilt, name):
    ...
    for i in C:
        ...
        x,y,z=cp.get_centroid(V,C,i)[0],cp.get_centroid(V,C,i)[1],cp.get_centroid(V,C,i)[2]
        if z>mid:
            if y>0:
                C.nodes[i]["region"]="ur"
            else:
